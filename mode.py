...
taiko_rpc_url = "https://mode.drpc.org"
...
gas_price_wrap_gwei = 0.00001  # Adjust as needed
gas_price_unwrap_gwei = 0.000003  # Adjust as needed
...
def wait_for_confirmation(tx_hash, timeout=300):
    ...
    while time.time() - start_time < timeout:  # Menggunakan detik untuk perbandingan
        try:
            ...
            if receipt:
                if receipt['status'] == 1:
                    print(Fore.GREEN + f"Transaction Successful | Tx Hash: {web3.to_hex(tx_hash)} | Network: Mode" + Style.RESET_ALL)  # Green
...
def wrap_eth_to_weth():
    ...
    try:
        ...
        print(Fore.BLUE + f"Transaction sent: Wrapping ETH to WETH | Amount: {web3.from_wei(amount_in_wei, 'ether')} WETH | Tx Hash: {web3.to_hex(tx_hash)} | Network: Mode" + Style.RESET_ALL)  # Blue
...
def unwrap_weth_to_eth():
    ...
    try:
        tx_hash = web3.eth.send_raw_transaction(signed_txn.raw_transaction)
        print(Fore.BLUE + f"Transaction sent: Unwrapping WETH to ETH | Amount: {web3.from_wei(amount_in_wei, 'ether')} ETH | Tx Hash: {web3.to_hex(tx_hash)} | Network: Mode" + Style.RESET_ALL)  # Blue
        if wait_for_confirmation(tx_hash):
            return True
    except Exception as e:
        print(Fore.RED + f"Transaction error: {e}" + Style.RESET_ALL)  # Red
...
try:
    while total_tx < 60:
        ...
        # Wrap ETH to WETH
        if wrap_counter < 30 and total_tx < 60:
            if wrap_eth_to_weth():
                wrap_counter += 1
                total_tx += 1
        ...
        # Unwrap WETH to ETH
        if unwrap_counter < 30 and total_tx < 60:
